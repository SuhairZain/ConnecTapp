...
ApplicationUI::ApplicationUI(bb::cascades::Application *app) : QObject(app)
{
	...
	tapMessage = Label::create().
			horizontal(HorizontalAlignment::Center).
			multiline(TRUE).
			//text("To share your profiles, add them in settings").
			textStyle(tapStyle);
	...
	sharePage = Page::create().
			content(shareRoot).
			addAction(saveAction, ActionBarPlacement::OnBar);
...
void ApplicationUI::onInvoked(const bb::system::InvokeRequest &request)
{
	...
	saveAction->setImage("asset:///images/icons/ic_save_as.png");
...
void ApplicationUI::setContentToShare()
{
	...
	{
		if(userLiID=="")
			tapMessage->setText("To share your Facebook profile, tap your phones");
		else
			tapMessage->setText("To share your Facebook and LinkedIn profiles, tap your phones");
	}
	else
	{
		tapMessage->setText("To share your LinkedIn profile, tap your phones");
	}
...
void ApplicationUI::setContentForHelp()
{
	...
	case 4:
		...
		helpLabel->setText("After profiles are received, they may be saved "
				"for later reference by tapping Save. The app allows you to "
				"add your current location while saving for your easiness.");
		break;
/*
	case 5:
...
void MyItemClass::updateItem(const QString text, const QString metText, const QString fb, const QString li)
{
		...
		const QString &type, const QVariantList &indexPath, const QVariant &data)
{
	...
	myItem->updateItem(map["name"].toString(), map["metPlace"].toString(), map["rfbID"].toString(), map["rliID"].toString());
}
