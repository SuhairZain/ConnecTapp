...
ApplicationUI::ApplicationUI(bb::cascades::Application *app) : QObject(app)
{
	...
	SegmentedControl *chooser = SegmentedControl::create().
			horizontal(HorizontalAlignment::Fill).
			add(Option::create().text("Share only")).
			add(Option::create().text("Send & Receive"));
	...
	ActionItem *review = ActionItem::create().
			...
			connect(SIGNAL(triggered()), this, SLOT(goToAppReviewPage()));
	...
	nfcEnable = ActionItem::create().
			image("asset:///images/icons/ic_nfc.png").
			title("Enable NFC").
			connect(SIGNAL(triggered()), this, SLOT(enableNFC()));
	...
	connect(app, SIGNAL(fullscreen()), this, SLOT(enableNFC()));
}
...
void ApplicationUI::enableNFC()
{
	nfc_set_setting(NFC_SETTING_ENABLED, TRUE);

	bool *result = new bool;
	nfc_get_setting(NFC_SETTING_ENABLED, result);

	if(*result)
	{
		nfcEnable->setEnabled(FALSE);
	}
	else
	{
		nfcEnable->setEnabled(TRUE);
		//enableNFC(); //Would be too annoying, wouldn't it? ;)
	}
...
void ApplicationUI::goToAppReviewPage()
{
	InvokeRequest invokeRequest;
...
void ApplicationUI::setContentToShare()
{
	...
	else
	{
		...
		tapMessage->setText("To share your profiles, please turn on NFC from app menu");
	}
